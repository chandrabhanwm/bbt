...
/** The user-facing level label — deliberately different from the
 *  internal `business.level` (1-6) used everywhere else for cost/income/
 *  synergy lookups. Buying a business sets internal level to 1, but
 *  that first purchase reads to a player as "I bought it," not "it's
 *  already at Level 1" — so the display shows "Owned" with no number
 *  there, and internal levels 2-6 display as Level 1-5. This is purely
 *  a label; every table lookup, synergy check, and cost calculation
 *  still uses the real internal level unchanged. */
export function getDisplayLevelLabel(internalLevel: number): string {
  if (internalLevel <= 1) return 'Owned';
  return `Level ${internalLevel - 1}`;
}
