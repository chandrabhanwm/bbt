...
import { Trophy, TrendingUp, Volume2, VolumeX, Award, ShieldAlert, Crown, Banknote } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="flex items-stretch gap-2">
            ...
            {editingName ? (
              ...
              <div onClick={() => { playClick(); setEditingName(true); }} className="cursor-pointer">
                <div className="flex items-center gap-1 min-w-0">
                  <span className="text-[12px] font-bold text-white uppercase tracking-wide truncate">
                    {playerName}
                  </span>
                  <Crown size={10} color={GOLD} fill={GOLD} className="flex-shrink-0" />
                </div>
        ...
        {/* ============ Cash balance pill ============ */}
        <div
          className="glossy-3d flex-shrink-0 flex flex-col items-center justify-center rounded-xl px-2 py-1"
        >
          <span className="text-[7.5px] font-bold tracking-wide whitespace-nowrap" style={{ color: GOLD }}>CASH BALANCE</span>
          <div className="flex items-center gap-1 mt-0.5">
            <Banknote size={13} color="var(--color-premium-green-500)" />
            <span className="text-[14px] font-bold whitespace-nowrap" style={{ color: GOLD_BRIGHT, fontVariantNumeric: 'tabular-nums' }}>
        ...
        {/* ============ Rank pill ============ */}
        <div
          className="glossy-3d flex-shrink-0 flex items-center gap-1.5 rounded-xl px-2 py-1"
        >
          <Trophy size={20} color={GOLD} className="flex-shrink-0" />
          <div className="flex flex-col leading-none">
            <span className="text-[11px] font-bold text-white whitespace-nowrap">#{stats.rank.toLocaleString('en-IN')}</span>
            <span className="text-[7px] font-bold tracking-wide mt-0.5" style={{ color: TEXT_SECONDARY }}>RANK</span>
          </div>
        </div>
        ...
        >
          {isMuted ? <VolumeX size={16} color="var(--color-premium-red-400)" /> : <Volume2 size={16} color={GOLD} />}
        </button>
      </div>
