  ...
  return (
    ...
    >
      <div className="flex flex-nowrap items-stretch gap-1.5">
            ...
            {editingName ? (
              ...
              <div onClick={() => { playClick(); setEditingName(true); }} className="cursor-pointer">
                ...
                <div className="flex items-center gap-1 mt-1 whitespace-nowrap">
                  ...
                  {/* Read-only mirror of the claim pool from the Portfolio
                      screen — no button here on purpose, just a glance at
                      "yes, this is building up." Claiming only happens on
                      Portfolio. */}
                  <span className="text-[9px] font-semibold whitespace-nowrap ml-1" style={{ color: GOLD }}>
                    · 🪙 {Math.round(stats.poolCash).toLocaleString('en-IN')}
                  </span>
                </div>
              </div>
            )}
        ...
        {/* ============ Rank pill ============ */}
        <div
          className="glossy-3d flex-shrink-0 flex flex-col items-center justify-center rounded-xl px-1 py-0.5"
        >
          <span className="text-[6.5px] font-bold tracking-wide whitespace-nowrap" style={{ color: GOLD }}>RANK</span>
          <div className="flex items-center gap-0.5 mt-0.5">
            <Trophy size={10} color={GOLD} className="flex-shrink-0" />
            <span className="text-[11px] font-bold text-white whitespace-nowrap" style={{ fontVariantNumeric: 'tabular-nums' }}>
