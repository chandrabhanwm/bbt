 ...
 * default, a soft gold highlight when the route is reachable. Active roads
 * also carry a small vehicle that continuously drives along them (native
 * SVG animateMotion, no extra JS animation loop needed) — this is what
 * makes the map read as a living city rather than a static diagram.
 * Locked/unreachable roads stay still — there's no reason for traffic to
 * be driving down a road nobody can actually take yet.
 */
export const RoadPath: React.FC<RoadPathProps> = ({ id, from, to, active, celebrating = false }) => {
  ...
  return (
    <g opacity={active ? 0.9 : 0.5}>
      ...
      {/* Small vehicle, continuously driving the route — active roads only */}
      {active && (
        <g>
          <image
