...
export const CityMapScreen: React.FC<CityMapScreenProps> = ({ onOpenDistrict, onPreviewDistrict, isDistrictUnlocked, districtProgress, businessesByDistrict, celebratingDistrictId }) => {
  ...
  return (
    <div className="relative w-full h-full overflow-hidden select-none" style={{ backgroundColor: 'var(--color-premium-bg)' }}>
      ...
      {/* Top label — same premium pill language as the Header, title adapted for this screen */}
      <div
        className="absolute top-3 left-3 z-30 flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold"
      ...
      >
        <Compass size={12} color="var(--color-premium-gold-400)" />
          ...
          <>
            ...
            <div className="absolute bottom-5 right-4 z-30 flex flex-col gap-2">
              <button
                ...
                aria-label="Zoom in"
              >
                <ZoomIn size={16} color="var(--color-premium-gold-400)" />
              </button>
              <button
                onClick={() => zoomOut()}
                className="w-10 h-10 rounded-xl flex items-center justify-center cursor-pointer active:scale-95 transition-transform"
                ...
                aria-label="Zoom out"
              >
                <ZoomOut size={16} color="var(--color-premium-gold-400)" />
              </button>
              <button
              ...
              >
                <Maximize size={15} color="var(--color-premium-gold-400)" />
              </button>
            </div>
          </>
        )}
