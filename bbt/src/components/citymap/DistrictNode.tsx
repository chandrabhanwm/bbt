...
export const DistrictNode: React.FC<DistrictNodeProps> = ({ district, onSelect, unlocked, progress }) => {
  ...
  return (
    ...
    >
      ...
      <foreignObject x="-22" y="-22" width="44" height="44" style={{ pointerEvents: 'none' }}>
        <div className="w-full h-full flex items-center justify-center">
          {iconFailed ? (
            <FallbackIcon size={20} strokeWidth={1.75} color={iconColor} />
          ) : (
            <img
              src={`/assets/district-icons/${district.id}.png`}
              alt=""
              className="w-full h-full object-contain"
              style={{ opacity: status === 'locked' ? 0.5 : 1, filter: 'drop-shadow(0 2px 3px rgba(0,0,0,0.4))' }}
