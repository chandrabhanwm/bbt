...
import {
  ...
  ShoppingBag, Flag, Lock, Star, Check, LucideIcon
} from 'lucide-react';
 ...
 * derived from data passed in (or the district's own static data as a
 * fallback), so dropping a new district into cityMapData.ts is all that's
 * needed to add another one of these.
 */
export const DistrictNode: React.FC<DistrictNodeProps> = ({ district, onSelect, unlocked, progress }) => {
  ...
  return (
    ...
    >
      ...
      {/* Green checkmark badge for completed districts */}
      {status === 'completed' && (
        <g transform="translate(18, -18)">
          <circle r="10" fill="var(--color-money-500)" stroke="#FFFFFF" strokeWidth="1.5" />
          <foreignObject x="-6" y="-6" width="12" height="12">
            <div className="w-full h-full flex items-center justify-center">
      ...
      {/* Tiny "COMPLETE" badge + 5-star row under the plaque, once completed */}
      {status === 'completed' && (
        <>
          <g transform="translate(0, 60)">
            <rect x="-22" y="-6" width="44" height="11" rx="5.5" fill="var(--color-brass-400)" stroke="#FFFFFF" strokeWidth="1" />
            <text textAnchor="middle" y="2.5" fontSize="6.5" fontWeight="800" fill="var(--color-ink-900)" fontFamily="Baloo 2, Inter, sans-serif">
              COMPLETE
            </text>
          </g>
          <g transform="translate(0, 75)">
            {Array.from({ length: 5 }).map((_, i) => (
              <Star
                ...
                fill={i < stars ? 'var(--color-brass-400)' : 'none'}
                strokeWidth={1.5}
              />
            ))}
          </g>
