...
interface DailyRewardCardsProps {
  ...
  /** Real wall-clock ms timestamp of the last card claim — gates the
   *  next not-yet-claimed card behind the same 60-second cooldown used
   *  for the Profit double-claim, creating the sequential "claim one,
   *  wait, claim the next" rhythm across all three cards. */
...
export const DailyRewardCards: React.FC<DailyRewardCardsProps> = ({ cards, onScratch, onClaim, lastCardClaimAt }) => {
  ...
  const [cooldownMessage, setCooldownMessage] = useState<{ index: number; seconds: number } | null>(null);
  ...
  const handleWatchAd = (index: number) => {
    const cooldownSeconds = getCooldownRemainingSeconds(lastCardClaimAt);
    if (cooldownSeconds > 0) {
      playClick();
      setCooldownMessage({ index, seconds: cooldownSeconds });
      setTimeout(() => setCooldownMessage((cur) => (cur?.index === index ? null : cur)), 1800);
      return;
    }
    playClick();
            ...
            <AnimatePresence mode="wait">
              {!card.scratched ? (
                ...
                >
                  ...
                  ) : cooldownMessage?.index === index ? (
                    <span className="text-[8.5px] font-bold" style={{ color: TEXT_SECONDARY }}>
                      ⏱️ Wait {cooldownMessage.seconds}s
                    </span>
                  ) : (
                    <button
