  ...
  return (
    <div className="w-full select-none">
      <div className="flex items-center gap-3 py-1">
        ...
        <span className="flex items-center gap-1.5 text-[13px] font-bold whitespace-nowrap" style={{ color: 'var(--color-premium-text)' }}>
            ...
            <AnimatePresence mode="wait">
              {!card.scratched ? (
                <motion.div
                  ...
                  style={{
                    background: 'linear-gradient(155deg, var(--color-premium-elevated) 0%, var(--color-premium-surface) 55%, var(--color-premium-bg) 100%)',
                  }}
                >
                  {/* Glossy top-left sheen — same shine language as the
                      ...
                      the shared class doesn't provide. */}
                  <div
                    className="absolute inset-0 pointer-events-none"
                    style={{ background: 'radial-gradient(130% 65% at 25% -10%, rgba(240,194,75,0.22) 0%, transparent 60%)' }}
                  />

                  {/* Soft circular spotlight behind the gift icon */}
                  <div
                    className="relative w-12 h-12 rounded-full flex items-center justify-center"
                    style={{ background: 'radial-gradient(circle, rgba(212,167,44,0.28) 0%, transparent 72%)' }}
                  >
                    <Gift size={22} color="var(--color-premium-gold-100)" strokeWidth={1.75} />
                  </div>

                  {/* Dashed "SCRATCH HERE" label, flanked by dash lines */}
                  <div className="relative flex items-center gap-1.5 px-2">
                    <div className="w-3 h-px" style={{ borderTop: '1.5px dashed var(--color-premium-text-secondary)' }} />
