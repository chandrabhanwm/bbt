    ...
    <div className="relative w-full h-full overflow-hidden">
      {imageUrl ? (
        ...
        <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, var(--color-premium-elevated) 0%, var(--color-premium-bg) 100%)' }}>
          <Icon size={200} strokeWidth={1} className="absolute -right-6 -top-10 opacity-[0.07]" color="var(--color-premium-gold-400)" />
        </div>
      )}
      ...
      <div className="absolute inset-x-0 bottom-0 h-16" style={{ background: 'linear-gradient(to top, var(--color-premium-overlay-fade), transparent)' }} />
    </div>
  );
});
