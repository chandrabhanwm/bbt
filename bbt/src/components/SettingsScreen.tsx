 ...
 */
export const SettingsScreen: React.FC<SettingsScreenProps> = ({ isOpen, onClose, playerName, onResetProgress, debugTrace }) => {
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="p-4 space-y-4 pb-10">
            {/* TEMPORARY — tracing the pool/signup inconsistency reported.
                Not meant to be permanent; removed once the real cause is found. */}
            <div className="rounded-2xl p-3.5" style={{ backgroundColor: 'var(--color-premium-surface)', border: '1.5px solid var(--color-premium-gold-400)' }}>
              <span className="text-[10px] font-bold uppercase tracking-wide" style={{ color: 'var(--color-premium-gold-400)' }}>
                Debug: Pool/Signup Trace (temporary)
              </span>
              <div className="mt-2 space-y-1 text-[11px]" style={{ color: 'var(--color-premium-text)' }}>
                <div>profitPerMin (actual): <b>{debugTrace.profitPerMin}</b></div>
                <div>Total businesses owned (all districts): <b>{debugTrace.totalBusinessesOwned}</b></div>
                <div>poolCash (actual): <b>{debugTrace.poolCash}</b></div>
                <div>hadNoLocalSaveAtBoot: <b>{String(debugTrace.hadNoLocalSaveAtBoot)}</b></div>
                <div>isBrandNewPlayer fired: <b>{String(debugTrace.isBrandNewPlayer)}</b></div>
                <div>UID: <b>{debugTrace.uid}</b></div>
                <div>App mounted at: <b>{new Date(debugTrace.mountedAt).toLocaleTimeString()}</b> ({Math.round((Date.now() - debugTrace.mountedAt) / 1000)}s ago)</div>
              </div>
            </div>
