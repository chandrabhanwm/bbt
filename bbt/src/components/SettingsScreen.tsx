...
interface SettingsScreenProps {
  ...
  googleLinkState: { status: 'idle' | 'working' | 'linked' | 'switched' | 'failed'; error: string | null };
  onLinkGoogle: () => void;
}
 ...
 */
export const SettingsScreen: React.FC<SettingsScreenProps> = ({ isOpen, onClose, playerName, onResetProgress, cloudDiagnostics, googleLinkState, onLinkGoogle }) => {
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="p-4 space-y-4 pb-10">
            ...
            <SectionLabel>Save Your Progress</SectionLabel>
            <Card>
              <div className="px-3.5 py-3">
                <p className="text-[11px] leading-relaxed mb-3" style={{ color: 'var(--color-premium-text-secondary)' }}>
                  Link a Google account so your progress follows you to any device — sign in with the same account later and get everything back.
                </p>
                {googleLinkState.status === 'linked' && (
                  <div className="text-[11px] font-semibold mb-2" style={{ color: 'var(--color-premium-green-500)' }}>
                    ✓ Linked! Your progress is now saved to this Google account.
                  </div>
                )}
                {googleLinkState.status === 'switched' && (
                  <div className="text-[11px] font-semibold mb-2" style={{ color: 'var(--color-premium-gold-400)' }}>
                    This Google account already had saved progress — you're now signed in to that account instead.
                  </div>
                )}
                {googleLinkState.status === 'failed' && (
                  <div className="text-[11px] font-semibold mb-2" style={{ color: 'var(--color-premium-red-400)' }}>
                    Couldn't link right now: {googleLinkState.error}
                  </div>
                )}
                <button
                  onClick={onLinkGoogle}
                  disabled={googleLinkState.status === 'working' || googleLinkState.status === 'linked' || googleLinkState.status === 'switched'}
                  className="w-full py-2.5 rounded-xl font-bold text-[12px] cursor-pointer"
                  style={{
                    backgroundColor: (googleLinkState.status === 'linked' || googleLinkState.status === 'switched') ? 'var(--color-premium-elevated)' : 'var(--color-premium-gold-400)',
                    color: (googleLinkState.status === 'linked' || googleLinkState.status === 'switched') ? 'var(--color-premium-text-secondary)' : 'var(--color-premium-text-inverse)',
                  }}
                >
                  {googleLinkState.status === 'working' ? 'Connecting…' : (googleLinkState.status === 'linked' || googleLinkState.status === 'switched') ? '✓ Google Account Linked' : 'Sign in with Google'}
                </button>
              </div>
            </Card>
