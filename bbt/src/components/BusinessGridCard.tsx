...
export const BusinessGridCard: React.FC<BusinessGridCardProps> = ({ business, index, imageUrl, onSelect }) => {
  ...
  return (
    ...
    >
      {/* Image region — clipped to rounded top corners locally, not via the
          ...
          Shrunk from 88px to make the whole card read as more compact. */}
      <div className="relative w-full h-[74px] rounded-t-[14px] overflow-hidden flex-shrink-0">
        {imageUrl ? (
      ...
      {/* Content region — flex-shrink-0 so this block, and everything in
          ...
          the name's 2-line reservation are untouched, since those were
          the actual fix for real cross-browser bugs, not just sizing. */}
      <div className="px-2 py-1 flex flex-col gap-[3px] flex-shrink-0">
        ...
        {/* Income row — restored per reference; money is always green,
            ...
            rotated color above it. */}
        <span className="flex items-center gap-1 text-[10px] font-semibold flex-shrink-0" style={{ color: 'var(--color-premium-green-500)' }}>
...
export const BusinessGridCardComingSoon: React.FC<{ name: string }> = ({ name }) => {
  return (
    <div
      ...
      style={{ minHeight: '148px' }}
    >
      <div className="relative w-full h-[74px] rounded-t-[14px] overflow-hidden flex-shrink-0 flex items-center justify-center" style={{ backgroundColor: 'var(--color-premium-elevated)' }}>
        <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: 'var(--color-premium-track)' }}>
          <Lock size={14} color="var(--color-premium-text-secondary)" />
        </div>
