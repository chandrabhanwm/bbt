...
export const BusinessGridCard: React.FC<BusinessGridCardProps> = ({ business, imageUrl, onSelect }) => {
  ...
  return (
    ...
    >
      {/* Image region — reduced 12px to free space for the business name below */}
      ...
      <div className="px-2 py-1 flex flex-col gap-[3px]">
        {/* Business name — reserves a fixed 2-line box so every card aligns
            ...
            No ellipsis, no truncation — wraps naturally instead. */}
        <span
...
export const BusinessGridCardComingSoon: React.FC<{ name: string }> = ({ name }) => {
  return (
    ...
    >
      <div className="relative w-full h-[80px] flex items-center justify-center" style={{ backgroundColor: 'var(--color-premium-elevated)' }}>
