...
import { AmbientBusinessEffect } from './AmbientBusinessEffect';
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full h-[64px] rounded-t-[14px] overflow-hidden flex-shrink-0">
        ...
        <AmbientBusinessEffect businessId={business.id} category={category.label} />

        {/* Buy/Upgrade badge, top-left — replaces the old category label
            with the actual action this card leads to, since a category
            name ("GROCERY") didn't tell a player anything actionable.
            Glows when currently affordable, so scanning the grid for
            "what can I get right now" doesn't require comparing numbers
            by hand. */}
        <motion.span
          className="absolute top-1.5 left-1.5 px-2.5 py-1 rounded-[6px] text-[9px] font-bold uppercase tracking-wide"
          style={{
            backgroundColor: isOwned ? 'var(--color-premium-badge-green)' : 'var(--color-premium-gold-400)',
            color: 'var(--color-premium-text-inverse)',
          }}
