  ...
  return (
    <div id="area-card" className="w-full select-none">
      <div
        ...
        style={{ backgroundColor: '#131110', border: `1.5px solid rgba(212,167,44,0.4)` }}
      >
        {/* Left: boost icon + title/subtitle, or watch-ad CTA */}
        {activeDoubleProfit ? (
          <div className="flex items-center gap-2 flex-shrink-0">
            <div className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: 'rgba(212,167,44,0.15)' }}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between text-[9px] font-bold uppercase tracking-wide" style={{ color: '#C9C6C1' }}>
            <span>{districtName} · {builtCount}/8 SHOPS</span>
            <span>{Math.round(progress)}%</span>
          </div>
          <div className="w-full h-[5px] rounded-full mt-1 overflow-hidden" style={{ backgroundColor: '#2A2620' }}>
            <motion.div
        ...
        {activeDoubleProfit && (
          <div className="flex flex-col items-center flex-shrink-0">
            <div className="w-9 h-9 rounded-full flex items-center justify-center" style={{ backgroundColor: 'rgba(212,167,44,0.15)', border: `1px solid ${GOLD}` }}>
              <Gift size={15} color={GOLD} />
            </div>
            <span className="text-[10px] font-bold mt-1" style={{ color: GOLD }}>{formatTimeShort(doubleProfitTimeRemaining)}</span>
          </div>
        )}
      </div>
