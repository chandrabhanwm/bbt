...
export const DistrictSummaryCard: React.FC<DistrictSummaryCardProps> = React.memo(({
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden" style={{ backgroundColor: '#131110', border: '1.5px solid rgba(212,167,44,0.4)' }}>
      ...
      <div className="relative h-[104px]">
        ...
        <div className="absolute inset-x-0 top-0 flex items-start justify-between p-3">
          ...
          <span className="flex items-center gap-0.5" aria-label={`${stars} of ${progressionConfig.maxStars} stars`}>
            {starSlots.map((_, i) => (
              <Star key={i} size={12} className={i < stars ? 'fill-current' : ''} color={i < stars ? GOLD : '#4A4640'} />
            ))}
          </span>
        </div>
      ...
      {/* Divider */}
      <div className="h-px" style={{ backgroundColor: 'rgba(212,167,44,0.25)' }} />
      ...
      {/* Progress bar + next-level text */}
      <div className="px-3 py-3">
        <div className="w-full h-[6px] rounded-full overflow-hidden" style={{ backgroundColor: '#2A2620' }}>
          <motion.div
...
const StatCell: React.FC<{ icon: React.ReactNode; label: string; value: string; valueColor?: string; last?: boolean }> = ({
  ...
  <div
    className="flex flex-col items-start gap-1 px-2.5 py-3"
    style={{ borderRight: last ? 'none' : '1px solid rgba(212,167,44,0.15)' }}
  >
    <span className="flex items-center gap-1 text-[7px] font-bold tracking-wide" style={{ color: GOLD }}>
      {label}
    </span>
    <span className="flex items-center gap-1 text-[11px] font-bold whitespace-nowrap" style={{ color: valueColor ?? '#FFFFFF' }}>
      {icon}
      {value}
    </span>
  </div>
