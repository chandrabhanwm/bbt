...
import { playCoin, playClick, playError } from '../utils/audio';
...
export const StreetView: React.FC<StreetViewProps> = ({
  ...
  }, [businesses, activeBubbles]);

  const handleCollectBubble = (bizId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    const bubble = activeBubbles[bizId];
    if (!bubble) return;

    playCoin();
    onReward(bubble.amount);
    ...
    const newFloat: TextFloat = {
      id: textId,
      bizId,
      text: `+₹${bubble.amount.toLocaleString('en-IN')}`,
  ...
  return (
    <div id="street-view-container" className="relative w-full rounded-3xl overflow-hidden border border-white/60 bg-white/40 shadow-md backdrop-blur-sm select-none">
      
      <div className="absolute top-3 left-3 z-30 flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-slate-950/80 border border-amber-400/30 backdrop-blur-md text-[9px] font-extrabold text-amber-300">
        <Sparkles size={10} className="animate-pulse text-amber-400" />
        <span>GANDHI NAGAR BAZAAR</span>
      </div>

      {/* Main Map Box Container (Guaranteed NO Horizontal Scrollbar, scales seamlessly) */}
      <div className={`relative w-full min-h-[350px] bg-gradient-to-b ${skyBackground} transition-all duration-1000 flex flex-col justify-between p-3 pt-10`}>
        ...
        <div className={`grid grid-cols-4 gap-2 h-full w-full items-stretch relative z-10 ${ambientLight} transition-all duration-1000`}>
          ...
          {columns.map((col, colIdx) => {
            const topBiz = businesses[col.topIdx];
            const bottomBiz = businesses[col.bottomIdx];
            ...
            return (
              <div key={colIdx} className="flex flex-col justify-between items-center h-full relative">
                ...
                <div 
                  ...
                  onClick={() => {
                    if (cash >= topBiz.cost) onUpgrade(topBiz.id);
                    else playError();
                  }}
                >
                ...
                <div 
                  ...
                  onClick={() => {
                    if (cash >= bottomBiz.cost) onUpgrade(bottomBiz.id);
                    else playError();
                  }}
                >
