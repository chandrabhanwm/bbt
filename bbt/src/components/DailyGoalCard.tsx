...
export const DailyGoalCard: React.FC<DailyGoalCardProps> = ({ goal, businessesByDistrict, districtName, onClaim }) => {
  ...
  return (
    <div className="glossy-3d rounded-2xl px-3.5 py-2.5 flex items-center gap-3">
      ...
      >
        <Target size={16} color="var(--color-premium-text-inverse)" />
      </div>

      <div className="flex-1 min-w-0">
        <div className="text-[9px] font-bold uppercase tracking-wide" style={{ color: TEXT_SECONDARY }}>
          Today's Goal
        </div>
        <div className="text-[12px] font-bold truncate" style={{ color: 'var(--color-premium-text)' }}>
          {label}
        </div>
        <div className="flex items-center gap-1.5 mt-0.5">
          <span className="text-[10px] font-semibold" style={{ color: complete ? GREEN : TEXT_SECONDARY }}>
            {progressText}
          </span>
          {!goal.claimed && (
            <span className="flex items-center gap-0.5 text-[9px] font-semibold" style={{ color: GREEN }}>
      ...
      {goal.claimed ? (
        <span className="text-[9px] font-bold uppercase tracking-wide flex-shrink-0" style={{ color: TEXT_SECONDARY }}>
          ✓ Claimed
        </span>
      ) : (
        <motion.button
          ...
          onClick={handleClaim}
          disabled={!complete}
          className="flex-shrink-0 px-3 py-2 rounded-xl font-bold text-[11px] cursor-pointer"
          style={{
