...
export const PortfolioScreen: React.FC<PortfolioScreenProps> = ({
  ...
  // Achievements read across every owned business now, not just the
  // current district — makes more sense for an empire-wide Portfolio
  // screen than the single-district scope this had as "Profile."
  ...
  const achievements = [
    {
      ...
      unlocked: (businessesByDistrict['badeban']?.find((b) => b.id === 'tea_stall')?.level ?? 0) >= 5,
      progress: Math.min(100, Math.round(((businessesByDistrict['badeban']?.find((b) => b.id === 'tea_stall')?.level ?? 0) / 5) * 100)),
    },
    {
      id: 'market_unlocked',
    ...
    },
    {
      ...
      desc: 'Claim the Daily Income Boost at least once.',
      unlocked: stats.lastDailyBoostClaimAt != null,
      progress: stats.lastDailyBoostClaimAt != null ? 100 : 0,
    },
    {
      id: 'wealth_builder',
  ...
  return (
    <div id="portfolio-tab" className="p-4 space-y-4 pb-28 select-none" style={{ backgroundColor: 'var(--color-premium-bg)' }}>
      ...
      <div
        ...
        style={{ backgroundColor: 'var(--color-premium-surface)', border: '1.5px solid var(--color-premium-border-strong)' }}
      >
        <span className="text-[9.5px] font-bold uppercase tracking-widest" style={{ color: TEXT_SECONDARY }}>
          Combined Income
        </span>
        <div className="flex items-center gap-1.5 mt-1">
          <CoinIcon className="w-4 h-4" premium />
          <span className="font-bold text-[22px]" style={{ color: GOLD }}>
            {formatCash(stats.profitPerMin)}<span className="text-[13px]" style={{ color: TEXT_SECONDARY }}>/min</span>
          </span>
        </div>
              ...
              <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="font-bold text-[18px] mt-0.5 flex items-center gap-1" style={{ color: 'var(--color-premium-text)' }}>
      ...
      <div className="grid grid-cols-3 gap-2.5">
        ...
        <BottomStat label="Net Worth" value={formatCash(netWorth)} />
      </div>
...
const BottomStat: React.FC<{ label: string; value: string }> = ({ label, value }) => (
  <div
  ...
  >
    ...
    <span className="font-bold text-[13px] mt-1" style={{ color: 'var(--color-premium-text)' }}>
      {value}
    </span>
  </div>
);
