...
import { subscribeToAuthChanges, auth, signOutUser, checkRedirectResult } from './firebase/config';
...
export default function App() {
  const [gateUnlocked, setGateUnlocked] = useState(() => localStorage.getItem('access_gate_unlocked') === 'true');
  ...
  // Sits in front of everything else, including the login screen — for
  // the testing phase where the concept itself needs to stay private,
  // not just individual accounts.
  if (!gateUnlocked) {
    return <AccessGateScreen onUnlocked={() => setGateUnlocked(true)} />;
  }
