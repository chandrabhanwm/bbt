...
import { EmpireStrip } from './components/EmpireStrip';
...
function AppInner() {
  ...
  const [stats, setStats] = useState<PlayerStats>(() => {
    ...
    if (saved) {
      ...
      return {
        ...
        lastUnlockedAchievementTitle: parsed.lastUnlockedAchievementTitle ?? null,
      };
    }
    return {
      ...
      lastUnlockedAchievementTitle: null,
    };
  });
  ...
  useEffect(() => {
    ...
    if (newlyUnlocked) {
      ...
      setTimeout(() => setMilestone(null), progressionConfig.celebrationDurationMs);
      setStats((prev) => ({ ...prev, lastUnlockedAchievementTitle: newlyUnlocked.title }));
    }
            ...
            >
              {activeTab === 'home' && (
                <div className="h-full flex flex-col gap-1.5 px-3 pt-1 pb-2">
                  <EmpireStrip
                    netWorth={stats.cash + getEmpireTotalInvested(businessesByDistrict)}
                    lastUnlockedAchievementTitle={stats.lastUnlockedAchievementTitle}
                  />
