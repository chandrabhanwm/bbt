...
import { Landmark, Sparkles, Award, RotateCcw, AlertCircle, RefreshCw } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-[#faf6f0] via-[#f4e7d3] to-[#e6d3b4] text-slate-800 flex flex-col items-center justify-center p-0 md:p-6 select-none overflow-hidden relative font-sans">
      ...
      <div className="relative w-full h-screen md:h-[880px] md:max-w-[420px] bg-[var(--color-vault-950)] md:rounded-[42px] md:border-[10px] md:border-[#523318] md:shadow-[0_24px_64px_rgba(82,51,24,0.45)] flex flex-col overflow-hidden">
        
        {/* Mobile Camera notch and status strip (Visible on desktop mockup shell only) */}
        <div className="hidden md:flex w-full h-8 bg-[#150d0a] justify-between items-center px-6 text-amber-500/80 text-[10px] font-mono font-bold select-none relative border-b border-[var(--color-vault-border)]">
          {/* Simulated Time */}
          <span>9:41 AM</span>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar bg-[var(--color-vault-950)] relative pb-28">
            ...
            >
              {activeTab === 'home' && (
                <div className="p-4 space-y-4">
                  {/* Gandhi Nagar progress and Video Sponsor Ad box */}
                  <AreaCard 
                  ...
                  {/* Interactive Isometric Street Sandbox View */}
                  <StreetView 
                    businesses={businesses} 
                    cash={stats.cash}
                  ...
                  {/* Business List upgrade section */}
                  <div className="bg-gradient-to-b from-[var(--color-vault-800)] to-[var(--color-vault-950)] rounded-t-[32px] p-4.5 -mx-4 mt-6 pb-6 space-y-4 border-t border-[var(--color-vault-border)] shadow-[0_-12px_36px_rgba(0,0,0,0.5)]">
                    <div className="flex items-center justify-between px-1">
                      <h3 className="font-display font-extrabold text-xs text-[var(--color-marigold-400)] uppercase tracking-widest flex items-center gap-1.5">
                        <Landmark size={13} className="text-[var(--color-marigold-400)]" />
                        <span>Your Businesses</span>
                      </h3>
                      <span className="text-[10px] text-slate-400 font-mono font-bold uppercase tracking-wider bg-[var(--color-vault-900)] px-2 py-0.5 rounded border border-[var(--color-vault-border)]">
                        Owned: {businesses.filter(b => b.level > 0).length} / 8
                      </span>
                    </div>
        ...
        <AnimatePresence>
          {levelUpMessage && (
            <motion.div
              ...
              className="absolute inset-x-8 top-1/3 z-50 p-5 court-card border-2 border-[var(--color-marigold-400)] rounded-3xl text-center shadow-2xl glow-marigold flex flex-col items-center overflow-visible"
            >
