...
import { subscribeToAuthChanges, auth } from './firebase/config';
...
function AppInner() {
  ...
  useEffect(() => {
    ...
    setCloudDiagnostics((prev) => ({ ...prev, signInStatus: uid ? 'success' : 'failed', signInError: uid ? null : 'No authenticated user found — this should not happen, since sign-in is required before the game loads.', uid }));
    ...
    if (uid && hadNoLocalSaveAtBootRef.current) {
      ...
      SaveService.cloudLoad(uid).then((cloudSave) => {
        ...
        if (cloudSave) {
          ...
          // (typed or otherwise) to overwrite.
