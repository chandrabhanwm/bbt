...
function AppInner({ currentUid }: { currentUid: string }) {
  // Definitive proof of whether this is a genuinely fresh component
  // instance for this account, or whether something is causing state to
  // survive across a sign-out/sign-in cycle. useRef's initial value
  // only ever runs once per real mount — if this timestamp is old on a
  // test that should be a brand-new mount, that's conclusive evidence
  // of the deeper bug, not just the race condition already fixed.
  ...
  const { handleSignOut, handleResetProgress, handleEstablishLegacy } = useAccountActions({
      ...
      <div className="app-shell-height relative w-full md:h-[880px] md:max-w-[420px] bg-[var(--color-ink-900)] md:rounded-[42px] md:border-[10px] md:border-[#523318] md:shadow-[0_24px_64px_rgba(82,51,24,0.45)] flex flex-col overflow-hidden">
        ...
        <SettingsScreen
          ...
          debugTrace={{
            ...
            totalBusinessesOwned: Object.values(businessesByDistrict).reduce((sum, list) => sum + list.filter((b) => b.level > 0).length, 0),
            poolCash: Math.round(stats.poolCash),
            hadNoLocalSaveAtBoot: hadNoLocalSaveAtBootRef.current,
            isBrandNewPlayer,
            uid: cloudUidRef.current ?? 'none',
