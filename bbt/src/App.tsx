...
import { ensureSignedIn, linkWithGoogle, auth } from './firebase/config';
...
function AppInner() {
  ...
  const hadNoLocalSaveAtBootRef = useRef(localStorage.getItem('basti_stats') === null);
  ...
  }>({ signInStatus: 'pending', signInError: null, uid: null, lastSaveStatus: 'never', lastSaveError: null, lastSaveAt: null });

  const [googleLinkState, setGoogleLinkState] = useState<{
    status: 'idle' | 'working' | 'linked' | 'switched' | 'failed';
    error: string | null;
  }>({ status: 'idle', error: null });

  /** "Sign in with Google" — links the player's current progress to a
   *  real account so it can follow them to any device. Two genuinely
   *  different outcomes, handled explicitly rather than treated the
   *  same: a fresh link keeps everything as-is; an already-used account
   *  means switching to that established save instead (using the exact
   *  same restore path as the boot-time cloud restore), and clearly
   *  telling the player that happened rather than silently swapping
   *  their save out from under them. */
  const handleLinkGoogle = async () => {
    setGoogleLinkState({ status: 'working', error: null });
    const result = await linkWithGoogle();

    if (!result.success) {
      setGoogleLinkState({ status: 'failed', error: result.error });
      return;
    }

    if (result.switchedAccount) {
      const newUid = auth.currentUser?.uid ?? null;
      cloudUidRef.current = newUid;
      setCloudDiagnostics((prev) => ({ ...prev, uid: newUid }));
      if (newUid) {
        const cloudSave = await SaveService.cloudLoad(newUid);
        if (cloudSave) {
      ...
      setGoogleLinkState({ status: 'switched', error: null });
    } else {
      setGoogleLinkState({ status: 'linked', error: null });
    }
  };
  ...
  useEffect(() => {
    let cancelled = false;
    ensureSignedIn().then(({ uid, error }) => {
      if (cancelled) return;
      cloudUidRef.current = uid;
      setCloudDiagnostics((prev) => ({ ...prev, signInStatus: uid ? 'success' : 'failed', signInError: error, uid }));

      if (uid && hadNoLocalSaveAtBootRef.current) {
      ...
      if (uid) {
        setCloudDiagnostics((prev) => ({ ...prev, lastSaveStatus: 'pending' }));
        SaveService.cloudSave(uid, latestSaveDataRef.current).then(({ ok, error: saveError }) => {
          setCloudDiagnostics((prev) => ({ ...prev, lastSaveStatus: ok ? 'success' : 'failed', lastSaveError: saveError, lastSaveAt: Date.now() }));
        });
      }
    });
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setCloudDiagnostics((prev) => ({ ...prev, lastSaveStatus: 'pending' }));
      SaveService.cloudSave(uid, latestSaveDataRef.current).then(({ ok, error }) => {
        setCloudDiagnostics((prev) => ({ ...prev, lastSaveStatus: ok ? 'success' : 'failed', lastSaveError: error, lastSaveAt: Date.now() }));
      });
