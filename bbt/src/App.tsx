...
function AppInner() {
  const { currentDistrictId, setCurrentDistrict, isDistrictUnlocked, unlockDistrict } = useDistrict();
  ...
  const currentDistrictProgress = useMemo(() => getDistrictProgress(businesses), [businesses]);
                ...
                <div className="h-full flex flex-col gap-3 p-3 pb-2">
                  ...
                  <div className="flex-1 min-h-0">
                    <StreetView
                      businesses={businesses}
                      cash={stats.cash}
                      districtLabel={`${currentDistrictMeta?.emoji ?? ''} ${currentDistrictMeta?.name ?? 'Unknown District'}`.trim()}
