...
// Daily Reward Cards: one reward tier per card slot, so variety is
// guaranteed rather than left entirely to chance. Card 3 has a small
// chance of a much bigger jackpot on top of its normal medium range.
const CARD_RESET_COOLDOWN_MS = 24 * 60 * 60 * 1000;
function generateRewardCard(slot: 0 | 1 | 2): RewardCard {
  let value: number;
  if (slot === 0) {
    value = 200 + Math.floor(Math.random() * 201); // ₹200–₹400
  } else if (slot === 1) {
    value = 600 + Math.floor(Math.random() * 401); // ₹600–₹1,000
  } else {
    const isJackpot = Math.random() < 0.1; // 10% chance
    value = isJackpot
      ? 3000 + Math.floor(Math.random() * 2001) // ₹3,000–₹5,000
      : 600 + Math.floor(Math.random() * 401);   // ₹600–₹1,000
  }
  return { scratched: false, value, claimed: false };
}
function generateFreshRewardCards(): RewardCard[] {
  return [generateRewardCard(0), generateRewardCard(1), generateRewardCard(2)];
}
                ...
                <div className="h-full flex flex-col gap-1.5 px-3 pt-1 pb-2">
                  ...
                  {isPreviewMode ? (
                    ...
                    <DailyRewardCards
                      ...
                      lastCardsResetAt={stats.lastCardsResetAt}
