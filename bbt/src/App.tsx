...
function AppInner({ currentUid }: { currentUid: string }) {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      // refresh the real top list and this player's real rank. Deliberately
      // only pushes name/avatar/net worth/level, never the full save
      // (see LeaderboardEntry in SaveService for why).
      const { businessesByDistrict: bbd, stats: currentStats, avatarEmoji: emoji, playerName: name } = latestSaveDataRef.current;
      const netWorth = currentStats.cash + getEmpireTotalInvested(bbd);
...
export default function App() {
  ...
  return (
    <DistrictProvider key={authUser.uid} currentUid={authUser.uid}>
      <AppInner currentUid={authUser.uid} />
    </DistrictProvider>
  );
}
