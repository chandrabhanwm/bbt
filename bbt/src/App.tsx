...
import { Landmark, Sparkles, RotateCcw, Lock, LayoutGrid, Map as MapIcon } from 'lucide-react';
...
function AppInner() {
  ...
  // UI-only toggle for how the current district's businesses are browsed —
  // 'grid' (card grid) or 'map' (the existing isometric StreetView). No
  // game logic reads or depends on this; it only decides what to render.
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-[#faf6f0] via-[#f4e7d3] to-[#e6d3b4] text-slate-800 flex flex-col items-center justify-center p-0 md:p-6 select-none overflow-hidden relative font-sans">
      ...
      <div className="relative w-full h-screen md:h-[880px] md:max-w-[420px] bg-[var(--color-ink-900)] md:rounded-[42px] md:border-[10px] md:border-[#523318] md:shadow-[0_24px_64px_rgba(82,51,24,0.45)] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar relative pb-28" style={{ backgroundColor: '#0A0A0B' }}>
              ...
              {activeTab === 'home' && (
                <div className="h-full flex flex-col gap-3 p-3 pb-2">
                  ...
                  {isPreviewMode ? (
                    ...
                    <div className="rounded-2xl px-3.5 py-2.5 flex items-center gap-2.5" style={{ backgroundColor: '#131110', border: '1.5px solid rgba(212,167,44,0.4)' }}>
                      <Lock size={16} className="flex-shrink-0" color="#8B8983" />
                      <div className="min-w-0">
                        ...
                        {displayedDistrictMeta?.unlockRequirement && (
                          <span className="block text-[9px] mt-0.5" style={{ color: '#8B8983' }}>
                            Unlock requirement: <span className="font-bold" style={{ color: '#D4A72C' }}>{displayedDistrictMeta.unlockRequirement.label}</span>
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  {/* Section header + Grid/Map toggle — UI-only state, both
                      views read the exact same businesses/onSelectShop contract */}
                  <div className="flex items-center justify-between">
                    <span className="text-[14px] font-bold text-white">
                      Businesses in {displayedDistrictMeta?.name ?? 'Unknown District'}
                    </span>
                    <div className="flex items-center gap-1 rounded-lg p-0.5" style={{ backgroundColor: '#131110', border: '1px solid rgba(212,167,44,0.3)' }}>
                      <button
                        onClick={() => { playClick(); setHomeViewMode('grid'); }}
                        className="flex items-center gap-1 px-2.5 py-1 rounded-md text-[10px] font-bold cursor-pointer"
                        style={{ backgroundColor: homeViewMode === 'grid' ? '#D4A72C' : 'transparent', color: homeViewMode === 'grid' ? '#0A0A0B' : '#8B8983' }}
                      >
                        <LayoutGrid size={11} /> Grid
                      </button>
                      <button
                        onClick={() => { playClick(); setHomeViewMode('map'); }}
                        className="flex items-center gap-1 px-2.5 py-1 rounded-md text-[10px] font-bold cursor-pointer"
                        style={{ backgroundColor: homeViewMode === 'map' ? '#D4A72C' : 'transparent', color: homeViewMode === 'map' ? '#0A0A0B' : '#8B8983' }}
                      >
                        <MapIcon size={11} /> Map
                      </button>
                    </div>
                  ...
                  {homeViewMode === 'grid' ? (
                    ...
                    /* The existing isometric street — untouched, same data/callback contract */
