...
import { playClick, playLevelUp } from './utils/audio';
...
function AppInner() {
  ...
  const [stats, setStats] = useState<PlayerStats>(() => {
    ...
    if (saved) {
      ...
      return {
        ...
        lastDailyBoostClaimAt: parsed.lastDailyBoostClaimAt ?? null,
      };
    }
    return {
  ...
  useEffect(() => {
    const tickInterval = setInterval(() => {
      setStats((prev) => {
        ...
        // Claim pool — same rate as cash, separate running total, capped
        // at 3 hours' worth. This is additive on top of cash, not instead
        // of it: cash keeps flowing in real time exactly as before.
  ...
  /** Daily Income Boost — flat reward, gated to once per 24 hours. Returns
   *  whether the claim actually succeeded, so the caller doesn't have to
   *  duplicate the cooldown check to know if it should show the reward. */
  const handleClaimDailyBoost = (): boolean => {
    const now = Date.now();
    const last = stats.lastDailyBoostClaimAt;
    if (last != null && now - last < DAILY_BOOST_COOLDOWN_MS) {
      return false;
    }
    setStats((prev) => ({
      ...prev,
      cash: prev.cash + DAILY_BOOST_REWARD,
      lastDailyBoostClaimAt: now,
    }));
    return true;
  };
  ...
  return (
    <div className="app-shell-height md:min-h-screen w-full bg-gradient-to-br from-[#faf6f0] via-[#f4e7d3] to-[#e6d3b4] text-slate-800 flex flex-col items-center justify-start md:justify-center p-0 md:p-6 select-none overflow-hidden relative font-sans">
      ...
      <div className="app-shell-height relative w-full md:h-[880px] md:max-w-[420px] bg-[var(--color-ink-900)] md:rounded-[42px] md:border-[10px] md:border-[#523318] md:shadow-[0_24px_64px_rgba(82,51,24,0.45)] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar relative pb-28" style={{ backgroundColor: 'var(--color-premium-bg)' }}>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {activeTab === 'home' && (
                <div className="h-full flex flex-col gap-1.5 px-3 pt-1 pb-2">
                  ...
                  {isPreviewMode ? (
                    ...
                    <AreaCard
                      ...
                      onClaimDailyBoost={handleClaimDailyBoost}
                      lastDailyBoostClaimAt={stats.lastDailyBoostClaimAt}
                    />
                  )}

                  <div className="glossy-3d rounded-2xl p-3 flex flex-col gap-3 flex-1 min-h-0">
                    {/* Section header — Grid is the only Home view for launch */}
                    <span className="text-[14px] font-bold text-white flex-shrink-0">
        ...
        <AnimatePresence>
          {levelUpMessage && (
            <motion.div
              ...
              style={{ backgroundColor: 'var(--color-premium-surface)', border: '2px solid var(--color-premium-gold-400)' }}
            >
              ...
              <h2 className="font-bold text-base uppercase tracking-widest" style={{ color: 'var(--color-premium-gold-400)' }}>
                Level Up!
              </h2>
        ...
        <AnimatePresence>
          {districtCompletedMessage && (
            <motion.div
              ...
              transition={{ duration: 0.25, ease: 'easeOut' }}
              className="absolute inset-x-8 top-1/3 z-50 p-5 rounded-3xl text-center flex flex-col items-center overflow-visible"
              style={{ backgroundColor: 'var(--color-premium-surface)', border: '2px solid var(--color-premium-green-500)' }}
            >
              {showConfetti && <Confetti />}
              ...
              <h2 className="font-bold text-base uppercase tracking-widest" style={{ color: 'var(--color-premium-green-500)' }}>
                District Completed!
              </h2>
              <p className="text-[11px] font-medium leading-relaxed mt-2" style={{ color: 'var(--color-premium-text)' }}>
                {districtCompletedMessage}
