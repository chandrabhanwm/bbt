...
function AppInner() {
  ...
  // Empire-wide count of owned businesses (across every district), for the
  // Profile screen's "Empire Summary" — same aggregation pattern as
  // playerDistrictsOwned above, not a new game mechanic.
  const totalBusinessesOwned = useMemo(() => {
    const allDistrictLists: Business[][] = Object.values(businessesByDistrict);
    return allDistrictLists.reduce((sum, list) => sum + list.filter((b) => b.level > 0).length, 0);
  }, [businessesByDistrict]);
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-[#faf6f0] via-[#f4e7d3] to-[#e6d3b4] text-slate-800 flex flex-col items-center justify-center p-0 md:p-6 select-none overflow-hidden relative font-sans">
            ...
            >
              {activeTab === 'home' && (
                <div className="h-full flex flex-col gap-3 p-3 pb-2">
                  ...
                  <DistrictSummaryCard
                    ...
                    districtIcon={displayedDistrictMeta?.icon ?? 'flag'}
                  ...
                  <div className="flex-1 min-h-0 flex flex-col gap-3 overflow-y-auto no-scrollbar">
                    <BusinessGridView
              ...
              {activeTab === 'profile' && (
                <ProfileTab 
                  ...
                  totalDistrictsOwned={playerDistrictsOwned}
      ...
      <div
        className="mt-4 flex gap-2 justify-center items-center md:hidden p-2.5 rounded-2xl"
      ...
      >
        <button 
          ...
          className="px-3 py-1.5 rounded-lg text-[9px] font-bold flex items-center gap-1 cursor-pointer"
          style={{ backgroundColor: 'var(--color-premium-elevated)', border: '1px solid var(--color-premium-green-500)', color: 'var(--color-premium-green-500)' }}
        >
          <span>💵 Cheat +₹1L</span>
        </button>
        <button 
          onClick={handleResetProgress}
          className="px-3 py-1.5 rounded-lg text-[9px] font-bold flex items-center gap-1 cursor-pointer"
          style={{ backgroundColor: 'var(--color-premium-elevated)', border: '1px solid var(--color-premium-red-400)', color: 'var(--color-premium-red-400)' }}
        >
          <span>🔄 Reset</span>
        </button>
      </div>
