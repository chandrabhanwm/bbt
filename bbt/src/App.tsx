...
import { getDistrictProgress, isDistrictCompleted, getDistrictCompletionReward } from './utils/districtProgress';
...
function AppInner() {
  ...
  const [poolClaimUI, setPoolClaimUI] = useState<{ amount: number; state: 'collected' | 'doubled' } | null>(null);
  ...
  const handleHeaderClaimPool = () => {
    ...
    setTimeout(() => setPoolClaimUI((cur) => (cur?.state === 'collected' ? null : cur)), 5000);
  };
  ...
  useEffect(() => {
    bastiCity.districts.forEach((district) => {
      ...
      setTimeout(() => setMilestone(null), progressionConfig.celebrationDurationMs);
  ...
  const handleClaimPool = (): number => {
    ...
    setStats((prev) => {
      ...
      const goalMatches = goal && !goal.claimed && goal.type === 'collect_pool_2' && claimed > 0;
      return {
        ...prev,
  ...
  return (
    <div className="app-shell-height md:min-h-screen w-full bg-gradient-to-br from-[#faf6f0] via-[#f4e7d3] to-[#e6d3b4] text-slate-800 flex flex-col items-center justify-start md:justify-center p-0 md:p-6 select-none overflow-hidden relative font-sans">
      ...
      <div className="app-shell-height relative w-full md:h-[880px] md:max-w-[420px] bg-[var(--color-ink-900)] md:rounded-[42px] md:border-[10px] md:border-[#523318] md:shadow-[0_24px_64px_rgba(82,51,24,0.45)] flex flex-col overflow-hidden">
        ...
        {/* Header pool claim overlay — floating, works on any tab, since
            claiming now happens from the Header itself, not just
            Portfolio. Same Collected!/Double it? flow as Portfolio's
            version, just presented as an overlay instead of inline in a
            card, since there's no dedicated space for it on every screen. */}
        <AnimatePresence>
          {poolClaimUI && (
            <motion.div
              ...
              className="absolute top-3 inset-x-6 z-40 rounded-2xl px-4 py-3 text-center glossy-3d"
            >
              {poolClaimUI.state === 'collected' ? (
                <>
                  <div className="text-[13px] font-bold flex items-center justify-center gap-1.5" style={{ color: 'var(--color-premium-green-500)' }}>
                    + {formatCash(poolClaimUI.amount)} ✓
                  </div>
                  <div className="font-bold text-[16px] mt-0.5" style={{ color: 'var(--color-premium-green-500)' }}>Collected!</div>
                  <button
                    onClick={handleHeaderDoubleClaim}
                    className="w-full mt-2.5 py-2 rounded-xl font-bold text-[11px] flex items-center justify-center gap-1.5 cursor-pointer"
                    style={{ backgroundColor: 'var(--color-premium-gold-400)', color: 'var(--color-premium-text-inverse)' }}
                  >
                    Double it? +{formatCash(poolClaimUI.amount)} more
                  </button>
                  <button
                    onClick={() => setPoolClaimUI(null)}
                    className="text-[9.5px] font-semibold mt-1.5 cursor-pointer"
                ...
                <div className="font-bold text-[14px]" style={{ color: 'var(--color-premium-green-500)' }}>
