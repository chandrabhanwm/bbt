...
export const districtEconomies: DistrictEconomy[] = [
  {
    ...
    properties: [
      { id: 'tea_stall', name: 'Chotu Chai Stall', emoji: '☕', baseCost: 5000, baseProfitPerMin: 100, description: 'Hot kadak chai for the whole market.' },
      { id: 'kirana_store', name: 'Sharma Kirana Store', emoji: '🛒', baseCost: 10000, baseProfitPerMin: 220, description: 'Daily groceries and household basics.' },
      { id: 'dairy_shop', name: 'Verma Dairy Shop', emoji: '🥛', baseCost: 20000, baseProfitPerMin: 450, description: 'Fresh milk, curd, and paneer every morning.' },
      { id: 'bakery', name: 'Basti Sweets & Bakery', emoji: '🍞', baseCost: 40000, baseProfitPerMin: 900, description: 'Warm bread, buns, and cream biscuits.' },
      { id: 'bike_repair', name: 'Chawla Autoparts', emoji: '🔧', baseCost: 80000, baseProfitPerMin: 1800, description: 'Spare parts and accessories for every two-wheeler.' },
      { id: 'medical', name: 'Gupta Medical Store', emoji: '💊', baseCost: 150000, baseProfitPerMin: 3500, description: 'Medicines and essentials, open late.' },
      { id: 'restaurant', name: 'Sher-e-Punjab Dhaba', emoji: '🍽', baseCost: 300000, baseProfitPerMin: 7500, description: 'Sit-down meals for the market crowd.' },
      { id: 'budget_lodge', name: 'Chaudhary Marriage Hall', emoji: '💒', baseCost: 600000, baseProfitPerMin: 15000, description: 'A grand venue for weddings and community functions.' },
    ],
  },
  {
    districtId: 'katra',
...
export function buildBusinessesForDistrict(districtId: string): Business[] {
  ...
  return economy.properties.map((p, i) => {
    ...
    // "Moment Zero" — Badeban's Tea Stall specifically is no longer
    // pre-owned. The very first business the player ever sees should be
    // something they actually buy, not something they start with. Every
    // other district's first-business behavior is deliberately left
    // exactly as it was — that wasn't the problem being solved here, and
    // changing it would be a broader, unrequested economy change.
    const isMomentZero = isFirst && districtId === 'badeban';
    ...
    return {
      ...
      level: isMomentZero ? 0 : (isFirst ? 1 : 0),
      status: isMomentZero ? 'locked' : (isFirst ? 'unlocked' : 'locked'),
