...
export function useCloudSync(params: UseCloudSyncParams) {
  ...
  useEffect(() => {
    ...
    // cheap Firestore WRITES (one document each), so a 30-second cadence
    // has no quota concern (widened from an original 10s for headroom
    // at a larger test group — see the interval value below).
